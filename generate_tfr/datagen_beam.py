...
def _create_example(pathuni):
    ...
    final["sequence"] = (
        np.reshape(final["sequence"], [51 * 10]).astype(np.float32).tolist()
    )
...
def _decode_tfexample(serialized_example):
    ...
    return {
        ...
        "class": np.reshape(eg.features.feature["class"].int64_list.value, [1])
        .astype(np.int64)
        .tolist(),
        "seq_len": eg.features.feature["seq_len"].int64_list.value[0],
        "sequence": np.reshape(
            eg.features.feature["sequence"].float_list.value, [51 * 10]
        )
        .astype(np.float32)
        .tolist(),
        "rendered": np.reshape(
            eg.features.feature["rendered"].float_list.value, [64 * 64]
        )
        .astype(np.float32)
        .tolist(),
    }
...
def _mean_to_example(mean_stdev):
    ...
    SIZE = 10
    mean_stdev["mean"] = (
        np.reshape(mean_stdev["mean"], [SIZE]).astype(np.float32).tolist()
    )
    mean_stdev["variance"] = (
        np.reshape(mean_stdev["variance"], [SIZE]).astype(np.float32).tolist()
    )
    mean_stdev["stddev"] = (
        np.reshape(mean_stdev["stddev"], [SIZE]).astype(np.float32).tolist()
    )
...
class MeanStddev(beam.CombineFn):
    ...
    def add_input(self, sum_count, new_input):
        ...
        new_sum_sq = np.sum([sum_sq, np.sum(np.power(new_input, 2), axis=0)], axis=0)
...
def create_glyphazzn_dataset(filepattern, output_path):
    ...
    def pipeline(root):
        ...
        (
            examples
            | "WriteToTFRecord"
            >> beam.io.tfrecordio.WriteToTFRecord(output_path, num_shards=NUM_SHARDS)
        )
...
def main(_):
    ...
    #     "binary_fp": binary / str,
    # }
    # pipeline_options = beam.options.pipeline_options.PipelineOptions(
    #     FLAGS.pipeline_options.split(",")
    # )
    PIO_DS_DIR = "../svg/trial/"
    TFR_DIR = "./tfrs/"
    STAT_PATH = "./stats/statsout"

    pipeline = create_glyphazzn_dataset(PIO_DS_DIR, TFR_DIR)
    with beam.Pipeline() as root:
        pipeline(root)

    pipeline = get_stats_of_glyphazzn(TFR_DIR + "*", STAT_PATH)
    with beam.Pipeline() as root:  # options=pipeline_options
        pipeline(root)
