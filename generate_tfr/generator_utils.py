...
def to_example(dictionary):
    ...
    for (k, v) in six.iteritems(dictionary):
        ...
        if six.PY3 and isinstance(v, map):
            v = list(v)
        if isinstance(v[0], six.integer_types) or np.issubdtype(type(v[0]), np.integer):
...
def shard_filepath(fname, num_shards):
    return [sharded_name(fname, shard, num_shards) for shard in range(num_shards)]
...
def maybe_download(directory, filename, uri):
    ...
    tf.logging.info(
        "Successfully downloaded %s, %s bytes." % (filename, statinfo.st_size)
    )
    return filepath
...
def get_or_generate_vocab(
    ...
    return get_or_generate_vocab_inner(
        data_dir, vocab_filename, vocab_size, vocab_generator, max_subtoken_length
    )
...
def get_or_generate_tabbed_vocab(
    data_dir, tmp_dir, source_filename, index, vocab_filename, vocab_size
):
    ...
    return get_or_generate_vocab_inner(data_dir, vocab_filename, vocab_size, generate())
...
def generate_dataset_and_shuffle(
    train_gen, train_paths, dev_gen, dev_paths, shuffle=True
):
...
class SequencePairPacker(object):
    ...
    def can_fit(self, pair, packed_length):
        return self._inputs.can_fit(pair[0], packed_length) and self._targets.can_fit(
            pair[1], packed_length
        )
    ...
    for example in examples:
        ...
        if chop_long_sequences and len(x) > packed_length:
            ...
            for i in range(num_fragments):
                yield packer(
                    x[packed_length * i : packed_length * (i + 1)], spacing
                ).to_dict()
...
def _pack_with_custom_ops(dataset, keys, length):
    ...
    Returns:
      a dataset.
    """
    from tensor2tensor.data_generators.ops import (
        pack_sequences_ops,
...
class SequenceDatasetPacker(object):
    ...
    def __init__(
        self, packed_length=256, spacing=0, queue_size=10, chop_long_sequences=False
    ):
    ...
    def _concurrent_pack(self, dataset, window_size=None, cycle_length=None, keys=None):
        ...
        return self._pack(
            dataset, window_size=window_size, cycle_length=cycle_length, keys=keys
        )
    ...
    def _pack(
        ...
        (dataset, self._num_sequences, self._token_dtype, keys) = self._standardize(
            dataset, keys
        )
        if window_size is None:
    ...
    def _eviction_fn(self, _):
        return tuple(
            -tf.ones((self._packed_length,), dtype=self._token_dtype)
    ...
    def _scanning_pack(self, dataset):
        ...
        if self._chop_long_sequences:
            ...
            dataset = dataset.filter(
                lambda *x: tf.reduce_max(  # pylint: disable=g-long-lambda
                    tf.stack([tf.shape(i)[0] for i in x]), axis=0
                )
                <= self._packed_length
            )
    ...
    def _finalize(self, _, contents):
        ...
        return {
            "contents": contents[:, : self._num_sequences],
            "segment": segment,
            "position": position,
        }
...
def _scan_step_fn(
    state, example, packed_length, queue_size, spacing, num_sequences, token_dtype
    ...
    if any_can_fit:
        ...
        shifted_range = (
            tf.range(queue_size, dtype=INDEX_DTYPE) - top_index
        ) % queue_size
        ...
        queue_index = (
            tf.reduce_min(shifted_range + exclusion_mask) + top_index
        ) % queue_size
        ...
        empty_contents = tf.zeros((packed_length, num_sequences * 2), dtype=token_dtype)
    ...
    padded_examples = [
        tf.pad(ex, padding[tf.newaxis, :]) for ex, padding in zip(example, pad_indices)
    ]
    padded_examples = tf.transpose(tf.stack(padded_examples))
    mask_update = tf.one_hot(
        start_index - leftmost, delta, dtype=contents.dtype, axis=0
    )

    content_update = tf.concat([padded_examples, mask_update], axis=1)

    index_range = (
        queue_index * packed_length
        + tf.range(delta, dtype=INDEX_DTYPE)  # Offset into the right section.
        + leftmost
    )
    contents = contents.scatter(
        index_range, contents.gather(index_range) + content_update
    )
...
def tfrecord_iterator_for_problem(
    problem, data_dir, dataset_split=tf.estimator.ModeKeys.TRAIN
):
